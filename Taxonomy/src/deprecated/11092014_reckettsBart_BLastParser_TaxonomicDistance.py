...
def TestSelfHit(subj, query, tax_dist_threshold):
    ...
    if s_txid == q_txid:
        return True
    try:
        s_tobj = TaxDB[str(s_txid)]
        q_tobj = TaxDB[str(q_txid)]
    except:
        print 'did not find OR \t %s \t %s'%(str(s_txid), str(q_txid))
        return False
...
if RunControl['mod1']:
    ...
    outDir = inUD
    fa_opath = os.path.join(outDir, 'safeintervals_%s_dist-%i_.fasta'%(root, DISTANCE_THRESH))
    results_opath = os.path.join(outDir, 'results-dist%i.txt'%DISTANCE_THRESH)
    
    hitLst = []
    oLst = [ProcessBLASTRun('dummy', [], [], 'dummy', LL=1)]#Labels
    
    fnLst = [['BO_DEZ_rickettsBart-bactrefseq_W15.txt', 'DEZ_rickettsBart.fasta']]
    for f in fnLst:
        print f
        bo_fn, bi_fn = f
