...
def processHits_InCommon(faObj, hitLst, ofp, hit_min_threshold = 0.7, NumSpeciesToTrac=1, verbose = False):
    ...
    if query != hitLst[0].query:
        raw_input('ERR')    
    seqIndexLst = [[0 for y in range(NumSpeciesToTrack+1)] for x in range(slen+1)]
    ...
    for h in hitLst:
        if h.subject == h.query:
            ...
            for i in range(h.qstart, h.qend, 1):
                seqIndexLst[i][species_index] = 1.0
            continue
    ...
    for x in range(slen+1):
        score = sum(seqIndexLst[x])/NumSpeciesToTrack
        seqIndexLst[x] = score
...
if RunControl['ProcessBlastRuns']:
    ...
    root = 'borellia-%0.2f'%HIT_THRESHOLD
    ...
    #hitLst = []
    oLst = [ProcessBLASTRun_Specific('dummy', [], [], 'dummy', LL=1)]#Labels
    fnLst = [['BO_borellia_vs_self.txt', 'DEZ_Borellia.fasta']]#2 Fill in run files
    for f in fnLst:
        print f
        bo_fn, bi_fn = f
        ...
        for line in BO_INP:
            ...
            if current_query == h.query:
                ...
                if results: oLst.append(results)
                #print 'New accumulation: ', h.query
                current_query = h.query
                faObj = FADict[h.query]
                procLst = [h]
                query_num +=1
            #if query_num>1000: break
        BO_INP.close()
        if procLst:
            if MODE == 'Specific':
                results = ProcessBLASTRun_Specific(f, faObj, procLst, ofp, LL=None)
