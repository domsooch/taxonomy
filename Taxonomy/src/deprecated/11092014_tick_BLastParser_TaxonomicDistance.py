...
def TestSelfHit(subj, query, tax_dist_threshold):
    ...
    try:
        s_txid = gi_tax[s_gi]
        #print 'found', s_gi, s_txid
    except:
        print 'No taxid: s_gi', s_gi
        return False
    try:
        q_txid = gi_tax[q_gi]
        #print 'found', q_gi, q_txid
    except:
...
if RunControl['mod1']:
    inUD = 'M:\\bacteria_refseq\\09252014_Run'#1: working directory
    hitLst = []
    oLst = [ProcessBLASTRun('dummy', [], [], 'dummy', LL=1)]#Labels
    
    fnLst = [['BO_tick-v-bactrefseq_W15.txt', 'DEZ_specdistance_seqs.fasta']]#2 Fill in run files
    for f in fnLst:
        print f
        bo_fn, bi_fn = f
