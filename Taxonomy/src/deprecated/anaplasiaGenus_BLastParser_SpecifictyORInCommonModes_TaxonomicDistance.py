...
if MODE == 'InCommon':
    HIT_THRESHOLD =0.4#1.0 #if nh < hit_threshold:
else:
    HIT_THRESHOLD =1.0 #if nh < hit_threshold:
DISTANCE_THRESH = 1 #genus  if s_tobj.Distance(q_tobj) <= DISTANCE_THRESH:
MIN_SEQ_SIZE = 100 #

RUN_LIMIT = 10000000
IGNORE_MISSING_TxID=True


root = 'anaplasigenusInCommon-%0.2f'%HIT_THRESHOLD
inUD = 'M:\\bacteria_refseq\\09252014_Run'
outUD = inUD

missing_gi_ofp = open('%s_missingGI.txt'%root, 'w')


fnLst = [['BO_anaplasiagenus_vs_self.txt', 'anaplasma_genus_768.fasta']]
seq_inpath = os.path.join(inUD, 'anaplasma_genus_768.fasta')
...
def TestSelfHit(subj, query, tax_dist_threshold):
    ...
    if s_txid == 'notfound':
        ...
        if IGNORE_MISSING_TxID:
            missing_gi_ofp.write('subj\t'+subj+'\t'+str(s_gi)+'\n')
            return True
        return False
    if q_txid == 'notfound':
        print 'notfound taxid for: query', q_gi
        if IGNORE_MISSING_TxID:
            missing_gi_ofp.write('query\t'+query+'\t'+str(q_gi)+'\n')
            return True
        return False
    if s_txid == q_txid:
        return True
...
if RunControl['prep_taxneigh']:
    ...
    if True:
        #Make GI to TaxObj Dict that will be used throughout
        import Tax.LabelFasta as LabelFasta
        giLst = LabelFasta.FASTAToGiLst(seq_inpath)
        for gi in giLst:
            tax_id = gi_tax[gi]
            species = TaxDB[tax_id].species
            
            #export tax neihbor lines
            neigborlineLst = TaxDB.export_TaxonDict(outpath='%s-nrtxNeigh.txt'%root)
            for line in neigborlineLst:
                tobj = TaxObj(line)
                tDB[str(tobj.taxid)] = tobj
                species = tobj.species
                if not(species in species_Dict):
    ...
    for blast_fn_set in fnLst:
        ...
        for line in BO_INP:
            ...
            if current_query == h.query:
                ...
                procLst = [h]
                query_num +=1
            if not(RUN_LIMIT==None) and query_num>RUN_LIMIT: break
        BO_INP.close()
        if procLst:
            if MODE == 'Specific':
                results = ProcessBLASTRun_Specific(f, faObj, procLst, ofp, LL=None)
