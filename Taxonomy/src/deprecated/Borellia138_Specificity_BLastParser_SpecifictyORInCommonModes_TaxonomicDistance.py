...
root = 'Borellia-%0.2f'%DISTANCE_THRESH
inUD = 'M:\\bacteria_refseq\\09252014_Run'
outUD = inUD

fnLst = [['BO_InCommonBorellia-v-bactrefseq_W15.txt','InCommonintervals_borellia-0.30.fasta'] for f in range(1,7,1)]
...
def TestSelfHit(subj, query, tax_dist_threshold, Verbose =False):
    ...
    try:
        ...
        if Verbose: print 'No taxid: s_gi', s_gi
        return False
    q_txid = gi_tax[q_gi]
    try:
        q_txid = gi_tax[q_gi]
    except:
        if Verbose: print 'No taxid: q_gi', q_gi
        return False
    if s_txid == 'notfound':
        if not(s_gi in missing_gi):
            missing_gi.append(s_gi)
        if Verbose: print 'notfound taxid for: subj', s_gi
        return False
    if q_txid == 'notfound':
        if not(q_gi in missing_gi):
            missing_gi.append(q_gi)
        if Verbose: print 'notfound taxid for: query', q_gi
        return False
    if s_txid == q_txid:
        return True
    s_tobj = TaxDB[s_txid]
...
if RunControl['prep_taxneigh']:
    ...
    tDB = {} 
    #CUSTOMIZE
    taxneigh_path = 'M:/bacteria_refseq/09252014_Run/edited_nrTaxNeighborhood_borellia.txt'
...
if RunControl['ProcessBlastRuns']:
    ...
    b = '\n'.join([str(g) for g in missing_gi])
    ofp = open('missing_giLst.txt', 'w')
    ofp.write(b)
    ofp.close()         
