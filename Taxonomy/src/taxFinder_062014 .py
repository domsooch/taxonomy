...
if RunControl['giToTax']:
    ...
    if False:
        workingUD = 'M:\\bacteria_refseq\\bact_fna\\'
        bactLst = os.listdir(workingUD)
        for fn in bactLst:
            if 'fna' in fn:
                print fn
                IFN = FA.path([workingUD, fn], fileType = 'fasta')
                LF.fagiToTax(tIFN, IFN, workingUD)
...
if RunControl['Nodes']:
    ...
    d = 'M:/bacteria_refseq/genarraytion_taxid/genarraytion_gi_taxid_nucl.dmp'
    IFN = FA.path(d,fileType = 'tab')
    indb = IFN.inDB()
    TaxonDict = {}
    for txID in indb:
        txID = txID[1]
